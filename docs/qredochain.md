...
Qredochain works on the principal of a ‘pegged sidechain’, Assets are transferred into the Qredochain using Peg-in, and similarly transferred out of Qredochain using the Peg-Out mechanism. All assets on the Qredochain represent locked assets on the original Cryptocurrency chain, as assets are destroy on the Qredochain, they are release back on the original Cryptocurrency chain. The Peg-in and Peg-out functionality is handled by a Multi Party Computation Cluster.
