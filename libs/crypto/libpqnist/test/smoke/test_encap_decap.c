...
int main()
{
    ...
    // Bob's SIKE keys
    uint8_t SIKEpk[NTHREADS][OQS_KEM_sike_length_public_key];
    uint8_t SIKEsk[NTHREADS][OQS_KEM_sike_length_secret_key];
    ...
    for(i=0; i<NTHREADS; i++)
    {
        ...
        if (rc)
        {
            ...
            OQS_MEM_cleanse(SIKEsk[i], OQS_KEM_sike_length_secret_key);
            exit(EXIT_FAILURE);
        }

        int j = OQS_KEM_sike_length_public_key;
        printf("Bob SIKE pklen %d pk: ", j);
        amcl_print_hex(SIKEpk[i], j);
        j = OQS_KEM_sike_length_secret_key;
    ...
    uint8_t ek[NTHREADS][OQS_KEM_sike_length_ciphertext];
