#define OQS_COMPILE_BUILD_TARGET "x86_64-pc-linux-gnu"
#define OQS_COMPILE_CFLAGS "-g -O2  -mmmx -mpopcnt -msse -msse2 -msse3 -msse4.1 -msse4.2 -mavx -mavx2 -maes  -std=gnu11 -Wall -Wextra -Werror  -Wimplicit -Wunused -Wcomment -Wchar-subscripts -Wuninitialized -Wstrict-prototypes -Wshadow -Wformat-security -Wwrite-strings  -Wno-missing-braces -Wno-missing-field-initializers -g  -fPIC  -fvisibility=hidden  -I/home/vagrant/liboqs/include -I.  -O3  -fomit-frame-pointer  -DCONSTANT_TIME "
#define OQS_COMPILE_LDFLAGS ""
/* #undef OQS_ENABLE_KEM_bike1_l1_cpa */
/* #undef OQS_ENABLE_KEM_bike1_l1_fo */
/* #undef OQS_ENABLE_KEM_bike1_l3_cpa */
/* #undef OQS_ENABLE_KEM_bike1_l3_fo */
/* #undef OQS_ENABLE_KEM_frodokem_1344_aes */
/* #undef OQS_ENABLE_KEM_frodokem_1344_shake */
/* #undef OQS_ENABLE_KEM_frodokem_640_aes */
/* #undef OQS_ENABLE_KEM_frodokem_640_shake */
/* #undef OQS_ENABLE_KEM_frodokem_976_aes */
/* #undef OQS_ENABLE_KEM_frodokem_976_shake */
/* #undef OQS_ENABLE_KEM_kyber_1024 */
/* #undef OQS_ENABLE_KEM_kyber_1024_90s */
/* #undef OQS_ENABLE_KEM_kyber_512 */
/* #undef OQS_ENABLE_KEM_kyber_512_90s */
/* #undef OQS_ENABLE_KEM_kyber_768 */
/* #undef OQS_ENABLE_KEM_kyber_768_90s */
/* #undef OQS_ENABLE_KEM_newhope_1024cca */
/* #undef OQS_ENABLE_KEM_newhope_512cca */
/* #undef OQS_ENABLE_KEM_ntru_hps2048509 */
/* #undef OQS_ENABLE_KEM_ntru_hps2048677 */
/* #undef OQS_ENABLE_KEM_ntru_hps4096821 */
/* #undef OQS_ENABLE_KEM_ntru_hrss701 */
/* #undef OQS_ENABLE_KEM_saber_firesaber */
/* #undef OQS_ENABLE_KEM_saber_lightsaber */
/* #undef OQS_ENABLE_KEM_saber_saber */
#define OQS_ENABLE_KEM_sidh_p434 1
#define OQS_ENABLE_KEM_sidh_p434_compressed 1
#define OQS_ENABLE_KEM_sidh_p503 1
#define OQS_ENABLE_KEM_sidh_p503_compressed 1
#define OQS_ENABLE_KEM_sidh_p610 1
#define OQS_ENABLE_KEM_sidh_p610_compressed 1
#define OQS_ENABLE_KEM_sidh_p751 1
#define OQS_ENABLE_KEM_sidh_p751_compressed 1
#define OQS_ENABLE_KEM_sike_p434 1
#define OQS_ENABLE_KEM_sike_p434_compressed 1
#define OQS_ENABLE_KEM_sike_p503 1
#define OQS_ENABLE_KEM_sike_p503_compressed 1
#define OQS_ENABLE_KEM_sike_p610 1
#define OQS_ENABLE_KEM_sike_p610_compressed 1
#define OQS_ENABLE_KEM_sike_p751 1
#define OQS_ENABLE_KEM_sike_p751_compressed 1
/* #undef OQS_ENABLE_SIG_dilithium_2 */
/* #undef OQS_ENABLE_SIG_dilithium_3 */
/* #undef OQS_ENABLE_SIG_dilithium_4 */
/* #undef OQS_ENABLE_SIG_mqdss_31_48 */
/* #undef OQS_ENABLE_SIG_mqdss_31_64 */
/* #undef OQS_ENABLE_SIG_picnic2_L1_FS */
/* #undef OQS_ENABLE_SIG_picnic2_L3_FS */
/* #undef OQS_ENABLE_SIG_picnic2_L5_FS */
/* #undef OQS_ENABLE_SIG_picnic_L1_FS */
/* #undef OQS_ENABLE_SIG_picnic_L1_UR */
/* #undef OQS_ENABLE_SIG_picnic_L3_FS */
/* #undef OQS_ENABLE_SIG_picnic_L3_UR */
/* #undef OQS_ENABLE_SIG_picnic_L5_FS */
/* #undef OQS_ENABLE_SIG_picnic_L5_UR */
/* #undef OQS_ENABLE_SIG_qTesla_p_I */
/* #undef OQS_ENABLE_SIG_qTesla_p_III */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_128f_robust */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_128f_simple */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_128s_robust */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_128s_simple */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_192f_robust */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_192f_simple */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_192s_robust */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_192s_simple */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_256f_robust */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_256f_simple */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_256s_robust */
/* #undef OQS_ENABLE_SIG_sphincs_haraka_256s_simple */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_128f_robust */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_128f_simple */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_128s_robust */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_128s_simple */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_192f_robust */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_192f_simple */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_192s_robust */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_192s_simple */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_256f_robust */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_256f_simple */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_256s_robust */
/* #undef OQS_ENABLE_SIG_sphincs_sha256_256s_simple */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_128f_robust */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_128f_simple */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_128s_robust */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_128s_simple */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_192f_robust */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_192f_simple */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_192s_robust */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_192s_simple */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_256f_robust */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_256f_simple */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_256s_robust */
/* #undef OQS_ENABLE_SIG_sphincs_shake256_256s_simple */
#define OQS_KEM_DEFAULT OQS_KEM_alg_sike_p434
#define OQS_MASTER_BRANCH /**/
#define OQS_SIG_DEFAULT OQS_SIG_alg_dilithium_2
#define OQS_VERSION_NUMBER 0x00201000L
#define OQS_VERSION_TEXT "0.2.1-dev"
#define USE_AES_C 1
#define USE_AES_INSTRUCTIONS 1
/* #undef USE_AES_OPENSSL */
#define USE_AVX2_INSTRUCTIONS 1
/* #undef USE_AVX512BW_INSTRUCTIONS */
/* #undef USE_AVX512DQ_INSTRUCTIONS */
/* #undef USE_AVX512F_INSTRUCTIONS */
#define USE_AVX_INSTRUCTIONS 1
/* #undef USE_BMI_INSTRUCTIONS */
/* #undef USE_FMA4_INSTRUCTIONS */
/* #undef USE_FMA_INSTRUCTIONS */
#define USE_MMX_INSTRUCTIONS 1
/* #undef USE_OPENSSL */
#define USE_POPCNT_INSTRUCTIONS 1
#define USE_SHA2_C 1
/* #undef USE_SHA2_OPENSSL */
#define USE_SHA3_C 1
/* #undef USE_SHA3_OPENSSL */
#define USE_SSE2_INSTRUCTIONS 1
#define USE_SSE3_INSTRUCTIONS 1
/* #undef USE_SSE4A_INSTRUCTIONS */
#define USE_SSE4_1_INSTRUCTIONS 1
#define USE_SSE4_2_INSTRUCTIONS 1
#define USE_SSE_INSTRUCTIONS 1
