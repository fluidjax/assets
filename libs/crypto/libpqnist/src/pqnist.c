...
int pqnist_sike_keys(char* seed, char* SIKEpk, char* SIKEsk)
{
    ...
    rc = OQS_KEM_keypair(kem, SIKEpk, SIKEsk);
    if (rc != OQS_SUCCESS)
    {
        return rc;
    }
...
int pqnist_encapsulate_encrypt(char* P, int Plen, char* IV, char* pk, char* ek)
{
    ...
    // AES-256 key
    uint8_t *K = NULL;
    K = malloc(kem->length_shared_secret);
...
#ifdef DEBUG
    int i = kem->length_shared_secret;
    printf("K %d K: ", i);
    amcl_print_hex(K, i);
#endif

    // Decrypt the ciphertext
    pqnist_aes_cbc_decrypt(K, PQNIST_AES_KEY_LENGTH, IV, C, Clen);
