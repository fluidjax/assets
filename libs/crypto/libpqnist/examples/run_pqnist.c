...
int main()
{
    ...
    i = OQS_KEM_sike_length_public_key;
    printf("Bob SIKE pklen %d pk: ", i);
    amcl_print_hex(SIKEpk, i);
    i = OQS_KEM_sike_length_secret_key;
    ...
    i = OQS_KEM_sike_length_ciphertext;
    printf("Alice ek1 %d ek1: ", i);
    amcl_print_hex(ek, i);
    printf("Alice ek2 %d ek2: ", i);
    amcl_print_hex(&ek[OQS_KEM_sike_length_ciphertext], i);
    printf("\n");
    ...
    // Alice signs message
    rc = pqnist_bls_sign(P.val, P.len, BLSsk, S);
    if(rc)
    {
    ...
    rc = pqnist_bls_verify(P.val, P.len, BLSpk, S);
    if (rc == BLS_OK)
    {
