...
int main()
{
    ...
    // Sign message
    rc = pqnist_bls_sign(M.val, M.len, sk1, sig1);
    if(rc != BLS_OK)
    {
    ...
    rc = pqnist_bls_sign(M.val, M.len, sk2, sig2);
    if(rc != BLS_OK)
    {
        fprintf(stderr, "ERROR pqnist_bls_sign rc: %d\n", rc);
        printf("FAILURE\n");
    ...
    // Verify message
    rc = pqnist_bls_verify(M.val, M.len, pk1, sig1);
    if (rc != BLS_OK)
    {
    ...
    rc = pqnist_bls_verify(M.val, M.len, pk2, sig2);
    if (rc != BLS_OK)
    {
        fprintf(stderr, "ERROR pqnist_bls_verify rc: %d\n", rc);
        exit(EXIT_FAILURE);
    ...
    rc = pqnist_bls_verify(M.val, M.len, pk12, sig12);
    if (rc != BLS_OK)
    {
        fprintf(stderr, "ERROR pqnist_bls_verify rc: %d\n", rc);
        exit(EXIT_FAILURE);
