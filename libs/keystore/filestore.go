...
func (f *FileStore) SetSeed(seed []byte) error {
	return f.Set("seed", seed)
}

// GetSeed returns the stored seed
func (f *FileStore) GetSeed() ([]byte, error) {
	return f.Get("seed")
}

// GetBLSKeys returns the BLS keys
func (f *FileStore) GetBLSKeys() (blsPublic []byte, blsSecret []byte, err error) {
	if f.kBp == nil {
		var keySeed []byte
		keySeed, err = f.GetSeed()
		if err != nil {
			return nil, nil, err
		}
		f.kBp, f.kBs, err = GenerateBLSKeys(keySeed)
		if err != nil {
			return
		}
	}

	return f.kBp, f.kBs, nil
}

// GetSIKEKeys returns the SIKE keys
func (f *FileStore) GetSIKEKeys() (sikePublic []byte, sikeSecret []byte, err error) {
	if f.kSp == nil {
		var keySeed []byte
		keySeed, err = f.GetSeed()
		if err != nil {
		...
		f.kSp, f.kSs, err = GenerateSIKEKeys(keySeed)
		if err != nil {
			return
		}
	}

	return f.kSp, f.kSs, nil
}
